 ...
 *
 * Instances are usually retrieved from [rememberGifDrawablePainter].
 *
 ...
 *
 * Forked from accompanist, so that it doesn't reset bounds each time.
 */
@Composable
fun rememberGifDrawablePainter(drawable: Drawable?): Painter = remember(drawable) {
    when (drawable) {
        null -> EmptyPainter
        else -> GifDrawablePainter(drawable)
    }
}
